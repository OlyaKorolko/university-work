...
Posts.push(new Post('Item Name21', 'post 21', new Date('May 8, 2016'), 'May Holmes', "https://i.stack.imgur.com/y9DpT.jpg", 24, 'London, England', ['hashtag3', 'hashtag7', 'hashtag1'], true));
...
class PostsLoader {
    createPost(post, container) {
        ...
        <button type="button" id="delete_post${post.id}" onclick="deletePost(${post.id})">DELETE POST</button>
    ...
    createInitialPosts() {
        ...
        for (var i = 0; i < initArr.length; i++) {
            postsloader.createPost(initArr[i], posts_ul);
        }
        if (this._checkForAccount(userName)) {
            for (var i = 0; i < initArr.length; i++) {
    ...
    _checkForAccount(userName) {
        if (userName === null) {
            document.getElementById('user_name').innerHTML = "";
            document.getElementById('add_advert_button_background').style.display = 'none';
            document.getElementById('log_out_button').innerHTML = 'LOG IN';
            document.getElementById('log_out_button_background').style.left = "180px";
            return false;
        } else return true;
    }
    ...
    createShowMoreButton() {
        ...
        var self = this;
        show_more_button.onclick = self.showMore;
    ...
    showMore() {
        var addArr = allArr.getPosts(allArr.shown_num, Math.min(10, allArr.posts.length - allArr.shown_num));
        for (var i = 0; i < addArr.length; i++) {
            postsloader.createPost(addArr[i], document.getElementById('posts'));
            if (addArr[i].isOwned) {
    ...
    browseNewPosts(filterConfig = {}) {
        ...
        if (newArr.length) {
            for (var i = 0; i < newArr.length; i++) {
                postsloader.createPost(newArr[i], posts_ul);
            }
...
function deletePost(id) {
    ...
    if (toDelete) {
        allArr.removePost(id);
        postsloader.browseNewPosts();
    }
}

function addPost(post) {
    if (allArr.addPost(post)) postsloader.browseNewPosts();
}

function editPost(id, post) {
    if (allArr.editPost(id, post)) postsloader.browseNewPosts();
}


document.getElementById('user_name').innerHTML = `${userName}`;
var allArr = new PostsHolder(Posts);
var filter = new Filter(document.getElementById('filter_form'));
var postsloader = new PostsLoader(document.getElementById('cont'));

filter.setFilter();
postsloader.createInitialPosts();
postsloader.createShowMoreButton();